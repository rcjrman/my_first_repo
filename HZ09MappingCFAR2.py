...
for entry in my_file:
	...
	a = my_df[['DIE_X','DIE_Y']]\
		[my_df['IVFE_TEST_0_Readback'] == 1]\
		[my_df['DAC_Test_1_Readback'] == 8]\
		[my_df['TEST_BIAS_0_Readback'] == 0]\
		[my_df['TEST_OSC_TRIM_Readback'] == 5]\
		[my_df['TEST_SPK_2_Readback'] == 39]\
		[my_df['TEST_SPK_OFFSET_TRIM_Readback'] == 17]\
		[my_df['TEST_BOOST_1_Readback'] == 0]\
		[my_df['TEST_BOOST_2_Readback'] == 17]\
		[my_df['TEST_BOOST_3_Readback'] == 131]\
		[my_df['TEST_BOOST_4_Readback'] == 127]\
		[my_df['TEST_BOOST_5_Readback'] == 0]\
		[my_df['TEST_BOOST_6_Readback'] == 10]\
		[my_df['TEST_BOOST_7_Readback'] == 0]\
		[my_df['TEST_BOOST_9_Readback'] == 204]\
		[my_df['TEST_BOOST_10_Readback'] == 32]\
		[my_df['TEST_BIAS_ICGM_TRIM_Readback'] == 0]\
		[my_df['TEST_BIAS_BANDGAP_TRIM_Readback'] == 63]\
		[my_df['TEST_IV_FE_I_TRIMF_Readback'] == 3]\
		[my_df['TEST_IV_FE_I_TRIMC_Readback'] == 18]\
		[my_df['TEST_IV_FE_I_TRIM_Readback'] == 1]\
		[my_df['TEST_IVADC_XTALK_COEFFICIENT_OTP'] == 2]\
		[my_df['TEST_IVADC_TEMP_COEFFICIENT_A_OT'] == 15]\
		[my_df['TEST_IVADC_TEMP_COEFFICIENT_B_OT'] == 0]\
		[my_df['TEST_MEAS_ADC_GAIN_TRIM_Readback'] == 1]\
		[my_df['TEST_MEAS_ADC_OFFSET_TRIM_Readba'] == 14]\
		[my_df['TEST_MEAS_ADC_TEMP_TRIM_Readback'] == 12]\
		[my_df['X_COORDINATE_LSB_Readback'] == 0]\
		[my_df['X_COORDINATE_MSB_Readback'] == 0]\
		[my_df['Y_COORDINATE_LSB_Readback'] == 0]\
		[my_df['Y_COORDINATE_MSB_Readback'] == 0]\
		[my_df['WAFER_NUMBER_LSB_Readback'] == 0]\
		[my_df['WAFER_NUMBER_MSB_Readback'] == 0]\
		[my_df['PART_NUMBER_LSB_Readback'] == 23]\
